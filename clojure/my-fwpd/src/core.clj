...
;;
;; 需要想到的是，这个过程包括两个步骤， 一，读取文件为字符串；二，字符串转为map
;;
;; 其中 二 步骤可以分为:
;; 2.1 将文件字符串通过回车符分割为行字符串集合，
;; 2.2 行字符串通过空格分割成单词集合，
;; 2.3 单词集合作为 vals与预定义的keys通过 zipmap 转换为map。
...
(defn file2map
  [file-string]
  (zipmap mapkeys (map #(clojure.string/split #"," %)
